...
public class Meal {
    ...
    public  static void mealRemoveAll(int bin) {
        String deleteMeals = "DELETE FROM Meal WHERE BIN = ?";
        try (PreparedStatement preparedStatement = BMS.getConnection().prepareStatement(deleteMeals)) {
            preparedStatement.setInt(1, bin);
            preparedStatement.executeUpdate();
            BMS.output("All related meals removed successfully.");
        } catch (SQLException e) {
            BMS.output("Error removing related meals: " + e.getMessage());
        }
    }
