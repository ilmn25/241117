...
public class Banquet {
    ...
    public static boolean availabilityUpdate(int bin) {
        try {
            ...
            boolean isAvailable = !LocalDate.now().isAfter(resultSet.getDate("Date").toLocalDate())
                    && !Admission.admissionQuotaFull(bin);
        ...
        try {
            ...
            while (updating) {
                ...
                BMS.output("0 = back\n1 = update name\n2 = update date\n3 = update time\n4 = update location\n" +
                        ...
                        "8 = update quota\n9 = update meal 1 (fish)\n10 = update meal 2 (chicken)\n" +
                        "11 = update meal 3 (beef)\n12 = update meal 4 (vegetarian)\n20 = DELETE BANQUET\n" +
                        "Note: avalibility is not to be manually set, it automatically updates based on whether\n " +
                        "the quota is full or expired (checked against real time.)\n" +
                        "Therefore, to cancel a banquet, delete it");
                ...
                if (updating) availabilityUpdate(bin);
            }
        } catch (SQLException | IllegalArgumentException e) {
    ...
    private static void banquetRemove(int bin) {
        String deleteBanquet = "DELETE FROM Banquet WHERE BIN = ?";
        try {
            Meal.mealRemoveAll(bin);

            try (PreparedStatement preparedStatement = BMS.getConnection().prepareStatement(deleteBanquet)) {
                preparedStatement.setInt(1, bin);
                preparedStatement.executeUpdate();
            }
            BMS.output("Banquet and all related meals removed successfully.");
        } catch (SQLException e) {
            BMS.output("Error during banquet removal: " + e.getMessage());
        }
    }
